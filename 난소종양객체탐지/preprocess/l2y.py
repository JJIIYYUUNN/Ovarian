...
# 전처리(모폴로지 적용)이미지 저장
def save_yolo_image_preprocess(json_data, json_path, image_dir_path, target_dir):
    '''Save yolo image to image_dir_path/target_dir'''
    json_name = os.path.basename(json_path)
    img_name = json_name.replace(".json", ".png")
    ...
    k = cv2.getStructuringElement(cv2.MORPH_RECT, (3,3)) # mopolyz
    if json_data["imageData"] is None:
        dirname = os.path.dirname(json_path)
        image_name = json_data["imagePath"]
        src_image_name = os.path.join(dirname, image_name)
        # src_image = cv2.imread(src_image_name)
        # grayscale + mopolyz
        src_image = cv2.imread(src_image_name, cv2.IMREAD_GRAYSCALE)
        opening = cv2.morphologyEx(src_image, cv2.MORPH_CLOSE, k, iterations=1)
        opening = cv2.morphologyEx(opening, cv2.MORPH_OPEN, k, iterations=1)
        cv2.imwrite(img_path, opening)
...
class Labelme2YOLO:
    ...
    def covert_json_to_text(self, target_dir, json_name):
        ...
        # if json_name.split("_")[1] == "SONO": # 초음파 이미지일 때,
        #     img_path = save_yolo_image_preprocess(json_data,
        #                                         json_path,
        #                                         self._image_dir_path,
        #                                         target_dir)
        # else: # CT이미지 일때,
...
if __name__ == '__main__':
    ...
    import argparse
    from label2labelme import Label2Labelme_SUM
    ...
    if data_type == "CT":
        convertor_1step = Label2Labelme_SUM(data_type, label_input_path, png_input_path, save_path)
        cancer_file_cnt, cancer_object_cnt, benign_file_cnt, benign_object_cnt = convertor_1step.change_label_structure()
        
    else:
        convertor_1step = Label2Labelme_SUM(data_type, label_input_path, png_input_path, save_path, task = "tumor_type")
